...
def multiple_parent_uniform_crossover(selected_parents, parents_per_offspring: int, population_size, toolbox, individual_array_size):
    """
    ...
    for mating_parents in list(splitted_parents_combined):
        for child_nbr in range(len(mating_parents[0])):
            offspring_weights = []
            offspring_sigmas = []
...
class EvolutionWrapper:
    ...
    def __init__(self, individual_array_size, mutation_operator: MutationOperators, mutation_arguments: dict, 
                 ...
                 selection_operator: SelectionOperators, run_number: str, method_number: str, num_clones: int):
        self._current_pop = []
    ...
    #def _create_offsprings(self, selected_parents):
     #   if (self._crossover_operator == CrossoverOperators.MULTIPARENTS_UNIFORM_CROSSOVER):
      #      return self._toolbox.crossover(selected_parents, 4,  self._population_size, self._toolbox, self._individual_array_size)
       # offsprings = list(map(self._toolbox.clone, selected_parents))
        #for offspring_one, offspring_two in zip(offsprings[::2], offsprings[1::2]):
         #   self._toolbox.crossover(offspring_one, offspring_two)
          #  self._clear_fitness_and_gain(offspring_one, offspring_two)
        #return offsprings

    def _create_offsprings(self, selected_parents):
        offsprings = [self._toolbox.clone(ind) for ind in selected_parents]*self._num_clones
        return offsprings
    ...
    def _evaluate_population(self):
        ...
        self._logger.info(f"Enemies beaten: {self._enemies_beaten_in_current_run}")
        
        results = [{ 'min': min(fits), 'max': max(fits), 'avg': np.average(fits), 'std': np.std(fits), 'generation_number': self._generation_number }]
        record.record_results(self._env.enemyn, self._method_number, self._run_number , results)
    ...
    def _fitness_and_gain(self, individual):
        fitness, player_life, enemy_life, time = self._env.play(pcont=individual)
        individual.fitness.values = [fitness]
        individual.gain = player_life - enemy_life
        if enemy_life == 0:
            self._enemies_beaten_in_current_run = self._enemies_beaten_in_current_run + 1
    ...
    def __initialize_toolbox(self):
        ...
        #self._toolbox.register("crossover", self._crossover_operator)
        self._toolbox.register("mutate", self._mutation_operator, **self._mutation_arguments)
        self._toolbox.register("select", self._selection_operator, k=int(self._population_size/self._num_clones), tournsize = 20, fit_attr='fitness')
